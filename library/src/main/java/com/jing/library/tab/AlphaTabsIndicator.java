...
public class AlphaTabsIndicator extends LinearLayout {
    ...
    public void setOnTabChangedListener(OnTabChangedListener listner) {
...
//                Log.i("info", "i:" + i);
//                int j = i;
//                if (j > mChildCounts/2f) {
//                    j--;
//                }
//                Log.i("info","j:"+j);
                //设置点击监听
                tabView.setOnClickListener(new MyOnClickListener(i - noTabViewCounts));
            } else {
//                throw new IllegalArgumentException("TabIndicator的子View必须是TabView");
                noTabViewCounts++;
            }
        }
        mChildCounts = mTabViews.size();
    ...
    private class MyOnClickListener implements OnClickListener {
        ...
        public void onClick(View v) {
            Log.i("info", "currentIndex:" + currentIndex);
            if (!(mTabViews.get(currentIndex) instanceof AlphaTabView)) return;
            //点击前先重置所有按钮的状态
            resetState();
    ...
    public void setItem(int item) {
        resetState();
        mTabViews.get(item).setIconAlpha(1.0f);
        if (null != mViewPager) {
